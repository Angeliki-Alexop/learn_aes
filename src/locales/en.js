    ...
    stepByStep: {
      ...
      matrix: {
        ...
        sboxOverlay: {
          ...
          sboxDescription: 'The S-box (Substitution box) is a fixed lookup table used in AES to replace each byte with a different byte during the encryption process. It introduces non-linearity to make the cipher resistant to patterns and attacks. Each input byte (in hex) selects a row and column in the S-box; the value at that position is the substituted (output) byte.',
          sboxHint: 'Hint (encryption): Click any cell to highlight its row and column. The selected cell shows the substituted value for the corresponding input byte.',
          invSboxTitle: 'Inverse S-box (decryption)',
          invSboxDescription: 'The Inverse S-box (Substitution box) is a fixed lookup table used in AES during the decryption process to reverse the SubBytes transformation. Each input byte (in hex) selects a row and column in the inverse S-box, the value at that position replaces the byte in the state. This step undoes the non-linear substitution applied during encryption and helps recover the original data.',
          invSboxHint: 'Hint (decryption): Click any cell to highlight its row and column. The selected cell shows the output value for the corresponding input byte.',
        },
